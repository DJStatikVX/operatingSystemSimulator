...
# 155 "OperatingSystem.c"
                               && i<20 && OperatingSystem_IsThereANewProgram() == 1; i++) {
  ...
  if (PID == -3) {
   ...
   ComputerSystem_DebugMessage(103,'e',programList[Heap_getFirst(arrivalTimeQueue, numberOfProgramsInArrivalTimeQueue)]->executableName);
  } else if (PID == -1) {
   OperatingSystem_ShowTime('e');
   ...
   OperatingSystem_ShowTime('e');
   ComputerSystem_DebugMessage(104,'e',programList[Heap_getFirst(arrivalTimeQueue, numberOfProgramsInArrivalTimeQueue)]->executableName,"--- invalid priority or size ---");
  } else if (PID == -4) {
   OperatingSystem_ShowTime('e');
   ComputerSystem_DebugMessage(105,'e',programList[Heap_getFirst(arrivalTimeQueue, numberOfProgramsInArrivalTimeQueue)]->executableName);
  } else if (PID == -5) {
   ComputerSystem_DebugMessage(144,'e',programList[Heap_getFirst(arrivalTimeQueue, numberOfProgramsInArrivalTimeQueue)]->executableName);
  } else {
...
void OperatingSystem_PrintReadyToRunQueue() {
 ...
 for (i = 0; i < 2; i++) {
  ...
  for (j = 0; j < numberOfReadyToRunProcesses[i]; j++) {
   if (j == 0) {
    ComputerSystem_DebugMessage(113, 's', readyToRunQueue[i][j].info, processTable[readyToRunQueue[i][j].info].priority);
   } else {
